...
pub async fn test_java_generic<G: Graph>() -> Result<(), anyhow::Error> {
    ...
    let main_import_body = format!(
        r#"package graph.mesh.java.controller;
...
import java.util.Optional;"#
    ...
    let main = imports
        ...
        .find(|i| {
            i.file
                == "src/testing/java/src/main/java/graph/mesh/java/controller/PersonController.java"
        })
        .unwrap();
    ...
    assert_eq!(
        ...
        "src/testing/java/src/main/java/graph/mesh/java/model/Person.java",
    ...
    .iter()
    .find(|e| e.name == "/person/{id}" && normalize_path(&e.file) == "src/testing/java/src/main/java/graph/mesh/java/controller/PersonController.java")
    .map(|n| Node::new(NodeType::Endpoint, n.clone()))
    .expect("GET /person/{id} endpoint not found");

    let post_person_endpoint = requests
    .iter()
    .find(|e| e.name == "/person" && normalize_path(&e.file) == "src/testing/java/src/main/java/graph/mesh/java/controller/PersonController.java")
    .map(|n| Node::new(NodeType::Endpoint, n.clone()))
    .expect("POST /person endpoint not found");

    let get_person_fn = functions
    .iter()
    .find(|f| f.name == "getPerson" && normalize_path(&f.file) == "src/testing/java/src/main/java/graph/mesh/java/controller/PersonController.java")
    .map(|n| Node::new(NodeType::Function, n.clone()))
    .expect("getPerson function not found");

    let create_person_fn = functions
    .iter()
    .find(|f| f.name == "createPerson" && normalize_path(&f.file) == "src/testing/java/src/main/java/graph/mesh/java/controller/PersonController.java")
    .map(|n| Node::new(NodeType::Function, n.clone()))
